...
(confirmed live: `Connected to {org ID} as {test username}`). Read as: the
...
reserved prefix, or re-upload the package with a different ID." Checked NuGet's own ID Prefix
Reservation docs (fetched live) rather than guess at the mechanics: once a prefix is reserved,
*every* submission matching it is rejected unless it comes from the account that reserved it —
there's no self-service "request access" flow; only the reservation owner can proactively
delegate a sub-prefix to another account. `Umbraco.*` is almost certainly reserved by the real
Umbraco HQ account (their own official packages — `Umbraco.Cms`, `Umbraco.Forms`, etc. — all live
under it), and this package was never affiliated with or published by Umbraco HQ — it only ever
mirrored their naming *convention* for third-party providers, which turns out to be a real
distinction NuGet enforces at the infrastructure level, not just a style choice.
...
fully clear from the `<description>` metadata ("Salesforce connection and actions for Umbraco
Automate") and every doc — nothing about *what this package is for* is hidden, only the ID no
...
is a separate, more disruptive decision — flagged, not done unprompted), and Salesforce action
aliases (`salesforce.createLead` etc.) — those were never derived from the package name, they're
plain Salesforce-object-name-derived strings.

**A real, subtle bug the rename exposed, not caused:** `SalesforceOAuthHandlers.cs` had
`context.Properties[OpenIddict.Constants.OAuthProperties.AccountLabel] = instanceUrl;` — no
`using Umbraco.Automate.OpenIddict;` anywhere in the file, and it still compiled, for years'
worth of builds in this session, because C#'s name resolution searches *enclosing* namespaces
outward when a name doesn't resolve directly: from `Umbraco.Automate.Salesforce.Configuration`,
the compiler walked outward to the enclosing `Umbraco.Automate` namespace, appended
`OpenIddict.Constants...`, and found the real `Umbraco.Automate.OpenIddict.Constants.OAuthProperties.AccountLabel`
type entirely by accidental namespace nesting — never through an explicit `using`. The instant
the namespace stopped nesting under `Umbraco.Automate` (this rename), that accidental resolution
path vanished and the build failed with `CS0234`. Fixed by fully-qualifying the reference
explicitly (`Umbraco.Automate.OpenIddict.Constants.OAuthProperties.AccountLabel`) rather than
re-establishing the accidental coupling — this was always fragile, rename or not.
...
didn't touch, since the old name there was backslash-escaped as regex syntax, not a plain literal
match — worth remembering for any future rename that this class of embedded-pattern reference
needs a manual check, a bulk substitution won't reliably catch it.