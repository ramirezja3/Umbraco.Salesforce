...
public class UpdateOpportunityStageActionTests
{
    ...
    private static UpdateOpportunityStageAction CreateAction(ISalesforceClient? client = null)
        => new(
    ...
    private static ActionContext ContextWith(UpdateOpportunityStageSettings settings, ConfiguredConnection? connection = null) => new()
    {
        AutomationId = Guid.NewGuid(),
        RunId = Guid.NewGuid(),
        StepId = Guid.NewGuid(),
        ActionAlias = "salesforce.updateOpportunityStage",
        Settings = settings,
        Connection = connection,
    };

    [Fact]
    public async Task ExecuteAsync_MissingOpportunityId_FailsValidation()
    {
        var action = CreateAction();

        var result = await action.ExecuteAsync(
            ContextWith(new UpdateOpportunityStageSettings { StageName = "Closed Won" }), CancellationToken.None);
    ...
    public async Task ExecuteAsync_MissingStage_FailsValidation()
    {
        ...
        var result = await action.ExecuteAsync(
            ContextWith(new UpdateOpportunityStageSettings { OpportunityId = "006xx0000000001" }), CancellationToken.None);
    ...
    public async Task ExecuteAsync_NoConnectionConfigured_FailsValidation()
    {
        ...
        var result = await action.ExecuteAsync(
            ContextWith(new UpdateOpportunityStageSettings { OpportunityId = "006xx0000000001", StageName = "Closed Won" }),
