...
namespace Automate.Salesforce.Connector.Configuration;
...
internal static class SalesforceOAuthHandlers
{
    ...
    internal sealed class ExtractInstanceUrl : IOpenIddictClientHandler<ProcessAuthenticationContext>
    {
        ...
        public ValueTask HandleAsync(ProcessAuthenticationContext context)
        {
            ...
            // token refreshes: OAuthCredentialsService.RefreshAccessTokenAsync calls
            // AuthenticateWithRefreshTokenAsync directly (no HttpContext, no
            // AuthenticationProperties), whose result exposes only
            // AccessToken/RefreshToken/AccessTokenExpirationDate — nothing reads
            // context.Properties[AccountLabel] back out on that path. Only the interactive callback
            // controller (OAuthCallbackController, via HttpContext.AuthenticateAsync's
            // AuthenticationProperties.Items) ever persists AccountLabel. Handling this on refresh
            // too would require a change inside Umbraco.Automate.OpenIddict, which is out of scope
            // for this package (docs/dev-notes.md §0) — so this is authorization_code-only, and the
            ...
            {
                context.Properties[Umbraco.Automate.OpenIddict.Constants.OAuthProperties.AccountLabel] = instanceUrl;
            }
