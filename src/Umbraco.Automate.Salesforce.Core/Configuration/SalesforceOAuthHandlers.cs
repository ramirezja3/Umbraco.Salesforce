...
/// token response field — the org's actual API base URL, which is <em>not</em> the login host
/// used to authenticate (see CLAUDE.md §0a). Salesforce's OAuth flow is otherwise standard OIDC,
...
internal static class SalesforceOAuthHandlers
{
    ...
    /// modifying <c>Umbraco.Automate.OpenIddict</c> (not allowed — see CLAUDE.md §0).
