...
**Because Salesforce genuinely needs its own persistence** (the Describe-metadata cache table in §5 — something Slack has no equivalent of), **model this package's project structure on `Umbraco.Automate.OpenIddict`'s split, not Slack's single-project shape**: OpenIddict ships as Core + `Persistence.SqlServer` + `Persistence.Sqlite` + a bundling meta-package that references both. That's the closest real precedent in this codebase for "a provider that needs its own tables," and §2/§9 below have been updated accordingly.
